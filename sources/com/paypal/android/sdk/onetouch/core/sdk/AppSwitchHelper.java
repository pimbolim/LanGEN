...
public class AppSwitchHelper {
    ...
    public static Result parseAppSwitchResponse(ContextInspector contextInspector, Request request, Intent intent) {
        ...
        } else if (extras.containsKey(ReactVideoView.EVENT_PROP_ERROR)) {
            request.trackFpti(contextInspector.getContext(), TrackingPoint.Error, (Protocol) null);
            return new Result((Throwable) new WalletSwitchException(extras.getString(ReactVideoView.EVENT_PROP_ERROR)));
        } else {
            request.trackFpti(contextInspector.getContext(), TrackingPoint.Error, (Protocol) null);
    ...
    private static Result processResponseIntent(Bundle bundle) {
        ...
        String string = bundle.getString(ReactVideoView.EVENT_PROP_ERROR);
        ...
        try {
            ...
            return new Result(string2, responseType, new JSONObject().put(PaymentMethodOptionsParams.Blik.PARAM_CODE, string3), bundle.getString("email"));
