...
class UtcDates {
    ...
    static String getTextInputHint(Resources resources, SimpleDateFormat simpleDateFormat) {
        ...
        if (pattern.replaceAll("[^y]", "").length() == 1) {
            pattern = pattern.replace(JWKParameterNames.ELLIPTIC_CURVE_Y_COORDINATE, "yyyy");
        }
        return pattern.replace("d", string3).replace("M", string2).replace(JWKParameterNames.ELLIPTIC_CURVE_Y_COORDINATE, string);
    }
