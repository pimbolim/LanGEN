...
class Poly1305 {
    ...
    private static long load32(byte[] bArr, int i) {
        return ((long) (((bArr[i + 3] & UByte.MAX_VALUE) << 24) | (bArr[i] & UByte.MAX_VALUE) | ((bArr[i + 1] & UByte.MAX_VALUE) << 8) | ((bArr[i + 2] & UByte.MAX_VALUE) << Tnaf.POW_2_WIDTH))) & 4294967295L;
    }
    ...
    static byte[] computeMac(byte[] bArr, byte[] bArr2) {
        ...
        if (bArr3.length == 32) {
            ...
            while (i3 < bArr4.length) {
                ...
                long load2610 = j8 + (load26(bArr5, 12, 8) | ((long) (bArr5[16] << 24)));
