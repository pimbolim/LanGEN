...
public final class EllipticCurves {
    ...
    public static boolean isValidDerEncoding(byte[] bArr) {
        ...
        int i5 = i + 3;
        if ((bArr[i5] & UByte.MAX_VALUE) >= 128) {
            return false;
        }
        if ((b2 > 1 && bArr[i5] == 0 && (bArr[i + 4] & UByte.MAX_VALUE) < 128) || bArr[i5 + b2] != 2) {
            return false;
        }
        ...
        int i6 = i + 5 + b2;
        if ((bArr[i6] & UByte.MAX_VALUE) >= 128) {
            return false;
        }
        return b3 <= 1 || bArr[i6] != 0 || (bArr[(i + 6) + b2] & UByte.MAX_VALUE) >= 128;
    }
