...
public class PaymentMethod {
    ...
    public static void getPaymentMethodNonces(final BraintreeFragment braintreeFragment, boolean z) {
        final Uri build = Uri.parse(TokenizationClient.versionedPath("payment_methods")).buildUpon().appendQueryParameter("default_first", String.valueOf(z)).appendQueryParameter("session_id", braintreeFragment.getSessionId()).build();
    ...
    public static void deletePaymentMethod(final BraintreeFragment braintreeFragment, final PaymentMethodNonce paymentMethodNonce) {
        ...
        try {
            jSONObject.put(CLIENT_SDK_META_DATA, new MetadataBuilder().sessionId(braintreeFragment.getSessionId()).source(PaymentMethodOptionsParams.WeChatPay.PARAM_CLIENT).integration(braintreeFragment.getIntegrationType()).build());
