...
public class PayPal {
    ...
    public static void createPaymentResource(BraintreeFragment braintreeFragment, PayPalRequest payPalRequest, boolean z, HttpResponseCallback httpResponseCallback) throws JSONException {
        ...
        JSONObject put = new JSONObject().put("return_url", checkoutRequest.getSuccessUrl()).put(CANCEL_URL_KEY, checkoutRequest.getCancelUrl()).put(OFFER_CREDIT_KEY, payPalRequest.shouldOfferCredit());
        ...
        if (!z) {
            put.put("amount", payPalRequest.getAmount()).put(CURRENCY_ISO_CODE_KEY, currencyCode).put(INTENT_KEY, payPalRequest.getIntent());
        ...
        if (payPalRequest.getShippingAddressOverride() != null) {
            ...
            jSONObject.put(PostalAddressParser.REGION_KEY, shippingAddressOverride.getRegion());
    ...
    private static <T extends Request> T populateRequestData(BraintreeFragment braintreeFragment, T t) {
        ...
        t.environment(str).clientId(clientId).cancelUrl(braintreeFragment.getReturnUrlScheme(), PayPalTwoFactorAuth.CANCEL_PATH).successUrl(braintreeFragment.getReturnUrlScheme(), "success");
        return t;
    }
