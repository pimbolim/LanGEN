...
public class GooglePayment {
        ...
        try {
            ...
            braintreeFragment.waitForConfiguration(new ConfigurationListener() {
                public void onConfigurationFetched(Configuration configuration) {
                    ...
                    PaymentsClient paymentsClient = Wallet.getPaymentsClient((Activity) braintreeFragment.getActivity(), new Wallet.WalletOptions.Builder().setEnvironment(GooglePayment.getEnvironment(configuration.getGooglePayment())).build());
    ...
    private static JSONObject buildCardTokenizationSpecification(BraintreeFragment braintreeFragment) {
        ...
        try {
            jSONObject2.put("gateway", "braintree").put("braintree:apiVersion", "v1").put("braintree:sdkVersion", BuildConfig.VERSION_NAME).put("braintree:merchantId", braintreeFragment.getConfiguration().getMerchantId()).put("braintree:metadata", new JSONObject().put("source", PaymentMethodOptionsParams.WeChatPay.PARAM_CLIENT).put("integration", braintreeFragment.getIntegrationType()).put("sessionId", braintreeFragment.getSessionId()).put("version", BuildConfig.VERSION_NAME).put("platform", "android").toString());
    ...
    private static JSONObject buildPayPalTokenizationSpecification(BraintreeFragment braintreeFragment) {
        JSONObject jSONObject = new JSONObject();
        try {
            jSONObject.put("type", "PAYMENT_GATEWAY").put("parameters", new JSONObject().put("gateway", "braintree").put("braintree:apiVersion", "v1").put("braintree:sdkVersion", BuildConfig.VERSION_NAME).put("braintree:merchantId", braintreeFragment.getConfiguration().getMerchantId()).put("braintree:paypalClientId", braintreeFragment.getConfiguration().getGooglePayment().getPaypalClientId()).put("braintree:metadata", new JSONObject().put("source", PaymentMethodOptionsParams.WeChatPay.PARAM_CLIENT).put("integration", braintreeFragment.getIntegrationType()).put("sessionId", braintreeFragment.getSessionId()).put("version", BuildConfig.VERSION_NAME).put("platform", "android").toString()));
        } catch (JSONException unused) {
        }
        return jSONObject;
    }
