...
class PdfFile {
    ...
    PdfFile(PdfiumCore pdfiumCore2, PdfDocument pdfDocument2, FitPolicy fitPolicy, Size size, int[] iArr, boolean z, boolean z2, int i, boolean z3, boolean z4, boolean z5) {
        this.showTwoPages = z;
    ...
    public void recalculatePageSizes(Size size) {
        ...
        for (Size calculate : this.originalPageSizes) {
            this.pageSizes.add(pageSizeCalculator.calculate(calculate, this.showTwoPages, this.isLandscape));
        }
        if (this.autoSpacing) {
    ...
    public float getPageSpacing(int i, float f) {
        return (this.autoSpacing ? this.pageSpacing.get(i).floatValue() : (float) this.spacingPx) * f;
    }
    ...
    public int getPageAtOffset(float f, float f2) {
        int i = 0;
        int i2 = 0;
        while (i < getPagesCount() && (this.pageOffsets.get(i).floatValue() * f2) - (getPageSpacing(i, f2) / 2.0f) < f) {
            i2++;
            i++;
        }
        int i3 = i2 - 1;
    ...
    public void renderPageBitmap(Bitmap bitmap, int i, Rect rect, boolean z) {
        ...
        this.pdfiumCore.renderPageBitmap(this.pdfDocument, bitmap2, documentPage(i), rect.left, rect.top, rect.width(), rect.height(), z);
    }
    ...
    public int determineValidPageNumberFrom(int i) {
        ...
        if (iArr != null) {
            if (i >= iArr.length) {
                pagesCount2 = iArr.length;
            }
            return i;
        }
        if (i >= getPagesCount()) {
            pagesCount2 = getPagesCount();
        }
        return i;
        return pagesCount2 - 1;
    }
