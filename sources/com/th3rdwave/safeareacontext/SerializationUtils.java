...
class SerializationUtils {
    ...
    static WritableMap rectToJsMap(Rect rect) {
        ...
        createMap.putDouble(JWKParameterNames.ELLIPTIC_CURVE_Y_COORDINATE, (double) PixelUtil.toDIPFromPixel(rect.y));
    ...
    static Map<String, Float> rectToJavaMap(Rect rect) {
        return MapBuilder.of("x", Float.valueOf(PixelUtil.toDIPFromPixel(rect.x)), JWKParameterNames.ELLIPTIC_CURVE_Y_COORDINATE, Float.valueOf(PixelUtil.toDIPFromPixel(rect.y)), "width", Float.valueOf(PixelUtil.toDIPFromPixel(rect.width)), "height", Float.valueOf(PixelUtil.toDIPFromPixel(rect.height)));
    }
}
