...
public class TagDescriptor<T extends Directory> {
    ...
    /* access modifiers changed from: protected */
    public String getFormattedFloat(int i, String str) {
    ...
    public String getBitFlagDescription(int i, Object... objArr) {
        ...
        for (int i2 = 0; objArr.length > i2; i2++) {
            ...
            if (strArr != null) {
                char c = (integer.intValue() & 1) == 1 ? (char) 1 : 0;
    ...
    public String get7BitStringFromBytes(int i) {
        ...
        while (true) {
            ...
            byte b = byteArray[i2] & UByte.MAX_VALUE;
    ...
    public String getShutterSpeedDescription(int i) {
        ...
        if (floatObject.floatValue() <= 1.0f) {
            DecimalFormat decimalFormat = new DecimalFormat("0.##");
            decimalFormat.setRoundingMode(RoundingMode.HALF_UP);
            return decimalFormat.format((double) (((float) Math.round(((double) ((float) (1.0d / Math.exp(((double) floatObject.floatValue()) * Math.log(2.0d))))) * 10.0d)) / 10.0f)) + " sec";
        }
        int exp = (int) Math.exp(((double) floatObject.floatValue()) * Math.log(2.0d));
        return "1/" + exp + " sec";
    }
