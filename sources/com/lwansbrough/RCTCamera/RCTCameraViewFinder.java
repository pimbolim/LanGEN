...
class RCTCameraViewFinder extends TextureView implements TextureView.SurfaceTextureListener, Camera.PreviewCallback {
    ...
    private synchronized void startCamera() {
        ...
        if (!this._isStarting) {
            ...
            try {
                Camera acquireCameraInstance = RCTCamera.getInstance().acquireCameraInstance(this._cameraType);
                this._camera = acquireCameraInstance;
                Camera.Parameters parameters = acquireCameraInstance.getParameters();
                int i = this._captureMode;
                boolean z2 = i == 0;
                if (i != 1) {
                    z = false;
                }
                ...
                if (barcodeAnyOrientation != null) {
                    ...
                    if (resultPoints != null) {
                        for (ResultPoint resultPoint : resultPoints) {
                            ...
                            createMap2.putString(JWKParameterNames.ELLIPTIC_CURVE_Y_COORDINATE, String.valueOf(resultPoint.getY()));
