...
public class AESGCM {
    ...
    private static void validate(byte[] bArr, int i) throws JOSEException {
        if (ByteUtils.safeBitLength(bArr) != 96) {
            throw new JOSEException(String.format("IV length of %d bits is required, got %d", new Object[]{96, Integer.valueOf(ByteUtils.safeBitLength(bArr))}));
        } else if (i != 128) {
            throw new JOSEException(String.format("Authentication tag length of %d bits is required, got %d", new Object[]{128, Integer.valueOf(i)}));
        }
    }
