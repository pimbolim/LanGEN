...
public class RSADecrypter extends RSACryptoProvider implements JWEDecrypter, CriticalHeaderParamsAware {
    ...
    public RSADecrypter(PrivateKey privateKey2) {
        this(privateKey2, (Set<String>) null, false);
    }
    ...
    public byte[] decrypt(JWEHeader jWEHeader, Base64URL base64URL, Base64URL base64URL2, Base64URL base64URL3, Base64URL base64URL4) throws JOSEException {
        ...
        if (base64URL == null) {
            ...
            if (algorithm.equals(JWEAlgorithm.RSA1_5)) {
                ...
                secretKey = RSA_OAEP_SHA2.decryptCEK(this.privateKey, base64URL.decode(), 256, getJCAContext().getKeyEncryptionProvider());
            } else if (algorithm.equals(JWEAlgorithm.RSA_OAEP_384)) {
                secretKey = RSA_OAEP_SHA2.decryptCEK(this.privateKey, base64URL.decode(), BitmapCounterConfig.DEFAULT_MAX_BITMAP_COUNT, getJCAContext().getKeyEncryptionProvider());
            } else if (algorithm.equals(JWEAlgorithm.RSA_OAEP_512)) {
                secretKey = RSA_OAEP_SHA2.decryptCEK(this.privateKey, base64URL.decode(), 512, getJCAContext().getKeyEncryptionProvider());
            } else {
