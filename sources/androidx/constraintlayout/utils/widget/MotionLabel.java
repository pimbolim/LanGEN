...
public class MotionLabel extends View implements FloatLayout {
    ...
    private float getVerticalOffset() {
        ...
        return ((((((Float.isNaN(this.mFloatHeight) ? (float) getMeasuredHeight() : this.mFloatHeight) - ((float) getPaddingTop())) - ((float) getPaddingBottom())) - ((fontMetrics.descent - fontMetrics.ascent) * f)) * (1.0f - this.mTextPanY)) / 2.0f) - (f * fontMetrics.ascent);
    }
    ...
    public void setTextSize(float f) {
        this.mTextSize = f;
        String str = TAG;
        Log.v(str, Debug.getLoc() + "  " + f + " / " + this.mBaseTextSize);
        TextPaint textPaint = this.mPaint;
