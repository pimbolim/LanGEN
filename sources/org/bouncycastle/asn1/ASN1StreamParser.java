...
public class ASN1StreamParser {
    ...
    public ASN1Encodable readObject() throws IOException {
        ...
        if (readLength >= 0) {
            ...
            if ((read & 192) == 192) {
                return new DLPrivate(z2, readTagNumber, definiteLengthInputStream.toByteArray());
            }
            if ((read & 64) != 0) {
                return new DLApplicationSpecific(z2, readTagNumber, definiteLengthInputStream.toByteArray());
            }
            if ((read & 128) != 0) {
                return new BERTaggedObjectParser(z2, readTagNumber, new ASN1StreamParser((InputStream) definiteLengthInputStream));
            }
            if (z2) {
                if (readTagNumber == 4) {
